...
for page in pages:
    ...
    url = f'https://yourlieinaprilmanga.com/manga/your-lie-in-april-chapter-{page}/'
    image_urls = web_scrape(url)
    chapter_size = save_images_to_folder(image_urls, f'from_{min_page}_to_{max_page}', page, convert_to_cbz)
    total_size += chapter_size

#print(f"Total size for all chapters: {total_size / (1024 * 1024):.2f} MB")

#ai_mode(f'D:/batcave/my_labo/soloLeveling/from_{min_page}_to_{max_page}')

convert_images_to_cbz(f"D:/batcave/my_labo/soloLeveling/from_{min_page}_to_{max_page}")
